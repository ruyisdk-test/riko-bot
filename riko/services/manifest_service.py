...
class ManifestService:
    ...
    def generate(up_name: str, gen_vers: List[str], down_grade: bool):
        ...
        # nvchecker result: {"name": ..., "event": "updated"|"unchanged", "old_version": ..., "version": ...}
        result = get_riko().get_nvchecker_result(up_name)

        # A missing record means this is likely a new package
        if result is None:
            logger.error("No such nvchecker upstream %s", up_name)
            logger.error("May be new package")
        else:
            # Default to the version detected by nvchecker
            if len(gen_vers) == 0:
                gen_vers.append(result["version"])

        if result is None:
            # Use an empty string as the old version marker for a new package
            # TODO: design a better version baseline mechanism
            old_ver = ""
        ...
        # Remove the old version to avoid regenerating it
        ...
        # packages-index stores historical manifests used as the base template
        gen_cbs: List[str] = riko_toml.get_combos()
        gen_cbs_ov: List[PackageVersion] = []  # old version for each combo

        for c in gen_cbs:
            if old_ver:
                pkg_ver = get_riko().get_packages_index_manifest(riko_toml.get_category(), c, old_ver)
                if pkg_ver is None:
                    # Fall back to the keep_back policy
                    if "keep_back" in riko_toml.get_policy(c):
                        # keep_back: use the latest manifest as the base
                ...
                # Use 0.0.0 as a placeholder for new packages
                # TODO: design a more complete version baseline mechanism:
                # TODO: * a manually maintained toml list
                # TODO: * an ignore list for upstream versions
        ...
        # riko.yaml is a required template
        ...
        def tree_parse_inner(key: str, value: Dict | List | str) -> Dict:
            if isinstance(value, Dict):
                ...
                # Keep plain alphanumeric strings unchanged
                if re.match(r"^[a-zA-Z0-9 ,\-+.]+$", value):
                    return {key: value}
                # Parse other strings into an expression AST
        ...
        def riko_yaml_ast_check(exp: ast.Expression, g_vars: Dict, g_calls: Dict) -> bool:
            ...
            # TODO: implement the full AST check
            return True

        def riko_yaml_run(_up, _ov: semver.Version, nm: Dict, ym: Dict) -> Dict:
            _upstream_version = nm["metadata"]["upstream_version"]
            _files = {}
            _label = []   # stack tracking the current position

            # Utility functions callable from riko.yaml expressions
            ...
            def _uncompress(_orig: str) -> str:
                ...
                # TODO: use `tar --list` for the exact uncompressed name
                for t in _tars:
                    if _orig.endswith(t):
                        return _orig[:-len(t)]
                return _orig
            ...
            def _ast_run(_ym_t: Dict):
                for k, v in _ym_t.items():
                    _label.append(k)  # push the current key
                    ...
                    if isinstance(v, ast.Expression):
                        ...
                        pass  # plain string, leave unchanged
                    ...
                    _label.pop()  # pop the current key
        ...
        def manifests_r1(_facts: Dict) -> bool:
            ...
            # Infer the strategy from the partition type
            if len(_map) == 1:
                if "disk" in _map.keys() or "live" in _map.keys():
                    ...
                    # Add extra partition mappings for Spacemit K1
        ...
        def manifests_r9(_facts: Dict) -> bool:
            ...
            for _d in _facts["distfiles"]:
                # Skip if checksums or size are already present
                ...
                try:
                    ...
                    # Always delete the downloaded file to free disk space
                    if _f_loc.exists():
                        _f_loc.unlink()
        ...
        def manifests_validate(_ma: Dict) -> bool:
            # Required fields; empty str = field must exist, empty list/dict = type check
            _key_must = {
                "format": "v1",
                "metadata": {
                    "desc": "",
                        ...
                        if isinstance(_v, str | List):
                            ...
                            if _rv is None:
                                if _type == "must":
                                    ...
                                    pass  # optional field may be absent
                            else:
                                if type(_rv) != type(_v):
        ...
        for gv in gen_vers:
            ...
            # Start from a copy of the original riko.yaml template
            riko_yaml = copy.deepcopy(riko_yaml_orig)

            riko_yaml_source = {}
            riko_yaml_cbs = {}
            manifest_ids = {}  # manifest_id per combo for DB records

            # Initialize base config from riko.yaml's "source" section
            ...
            for i in range(0, len(gen_cbs)):
                if gen_cbs[i] in riko_yaml.keys():
                    ...
                    # Merge combo-specific config into the base config
                    ...
                    # Record the manifest generation in the DB
                    manifest_id = None
                    try:
            ...
            # riko.py may define custom rikoring / post_rikoring hooks

            riko_py_rikoring = None
            riko_py_post_rikoring = None
            if riko_py_p.exists():
                # Dynamically load the riko.py module
                ...
                # rikoring runs before manifest generation
                try:
                    riko_py_rikoring = getattr(riko_py_module, "rikoring")
                except AttributeError as e:
                    logger.debug(e)

                # post_rikoring runs after manifest generation
                try:
            ...
            # rikoring can modify packages before generation
            ...
            # Run inference rules to fill in missing manifest info
            ...
            # post_rikoring runs after validation
            ...
            # keep_back: keep the old version when checksums match
            for i in range(0, len(new_versions)):
                if old_versions[i].accept_policy("keep_back"):
                    ...
                    # Mark not ready when checksums match
                    if same:
            ...
            for v in new_versions:
                ...
                # -trunk versions use the full upstream_version as the filename
                ...
                # Mark the DB record as successful
                combo_name = v.get_combo()
                if combo_name in manifest_ids:
                    try:
